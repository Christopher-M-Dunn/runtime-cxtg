...
- [cxsdata_csr_op](cx.c/cxsdata_csr_read.md) — combined `.op` handler for all cxsdata variants (csrr/csrw/csrrw/csrrs/csrrc); reads old value if `ret_value != NULL`, writes if `write_mask != 0`, then increments `cxsidx` exactly once regardless of variant
